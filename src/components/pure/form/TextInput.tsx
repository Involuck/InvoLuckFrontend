...
export function TextInput({ size = 'md', state = 'default', className = '', ...props }: TextInputProps) {
  const base = 'block w-full rounded-md border bg-white text-gray-900 placeholder:text-gray-400 outline-none disabled:opacity-60 disabled:cursor-not-allowed transition-colors';
  const cls = `${base} ${sizeClasses[size]} ${stateClasses[state]} ${className}`;
  return <input className={cls} {...props} aria-invalid={state === 'error' || props['aria-invalid']} />;
}
